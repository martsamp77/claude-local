<#
.NAME        scantopdf-dashboard.lib
.SYNOPSIS    Data collection + HTML rendering for scantopdf-dashboard.ps1. Dot-sourced by it; NOT run directly.
.PLATFORM    windows
.CATEGORY    monitoring
.WHEN        Edit THIS file to change what the dashboard shows or how it looks. The running -Serve server
             watches this file and hot-reloads it on save - your change appears on the next page refresh,
             no restart needed. (Server plumbing - the listener loop, params, snapshot writing - lives in
             scantopdf-dashboard.ps1 and does need a restart if changed.)
#>
# Functions here run in the dashboard script's scope, so they read its params ($ServiceName, $ConfigDir,
# $SourceFolder, $ShowFilenames, ...) directly. Keep this file side-effect-free: define functions only.
